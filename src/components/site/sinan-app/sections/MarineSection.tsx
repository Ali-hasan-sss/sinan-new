...
import { sinanMarineLogoPng, SINAN_SITE_PL, SINAN_SITE_PX } from "../constants";
...
export function MarineSection({
  ...
  sectionInViewMarine: boolean;
}) {
  const { t } = useLanguage();

  return (
    <section
      ref={assignSectionRef("marine")}
      id="marine"
    ...
    >
      ...
      <motion.div
        className="hidden sm:block absolute top-[6%] md:top-[5%] lg:top-[4%] xl:top-[3%] pointer-events-none w-[min(48vw,460px)] lg:w-[min(54vw,580px)] xl:w-[min(56vw,660px)] 2xl:w-[min(58vw,720px)] max-h-[min(38vh,360px)] lg:max-h-[min(44vh,480px)] xl:max-h-[min(48vh,540px)] 2xl:max-h-[min(52vh,600px)] flex items-start justify-end"
        style={{ zIndex: 5, right: "var(--sinan-site-padding-x)" }}
        initial={false}
        animate={
          sectionInViewMarine
        ...
        transition={{
          duration: 1,
          delay: sectionInViewMarine ? 0.25 : 0,
          ease: [0.22, 1, 0.36, 1],
        }}
      >
        <img
          src={staticImageSrc(submarineImage)}
          alt=""
      ...
      >
        ...
        >
          ...
          <motion.p
            className="text-white text-center mb-3 md:mb-5 lg:mb-5 xl:mb-6 leading-relaxed mx-auto max-w-[52%] lg:max-w-[58%] xl:max-w-[60%] text-xl md:text-2xl lg:text-[clamp(1.75rem,1.45rem+0.45vw,2.45rem)] xl:text-[clamp(1.9rem,1.55rem+0.5vw,2.65rem)] 2xl:text-[clamp(2rem,1.65rem+0.55vw,2.85rem)] font-bold whitespace-pre-line"
            style={{
              fontFamily: "DIN Arabic, sans-serif",
            }}
            initial={false}
            ...
            transition={{
              duration: 0.55,
              delay: sectionInViewMarine ? 0.15 : 0,
              ease: [0.22, 1, 0.36, 1],
            }}
          >
          ...
          <motion.p
            className="text-white/90 text-center leading-relaxed mx-auto max-w-[85%] text-base md:text-lg lg:text-[clamp(1.1rem,1rem+0.28vw,1.45rem)] xl:text-[clamp(1.15rem,1.05rem+0.32vw,1.55rem)] 2xl:text-[clamp(1.2rem,1.1rem+0.36vw,1.65rem)] font-normal whitespace-pre-line"
            style={{
              fontFamily: "DIN Arabic, sans-serif",
            }}
            initial={false}
            ...
            transition={{
              duration: 0.55,
              delay: sectionInViewMarine ? 0.28 : 0,
              ease: [0.22, 1, 0.36, 1],
            }}
          >
            {t.marine.intro.split("\n\n")[1] ?? ""}
